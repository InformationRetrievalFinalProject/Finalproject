...
def topViewAndRankByTitle(query_dict, alpha):
  ...
  for term, value in query_dict.items():
    ...
    for doc, weight in docTf:
      simDoc[doc] += alpha * (1 + index_views[doc] + index_pageRanks[doc])
...
def search():
    ...
    if len(query_dict) == 1:
      simDoc = topViewAndRankByTitle(query_dict, 1)
    
    else:
      simDocByAnchorText = topByAnchorText(query_dict, 0.25)
      simDocByAnchorTitle = topViewAndRankByTitle(query_dict, 0.15)
      simDoc = calculateBM25(query_dict, simDocByAnchorText + simDocByAnchorTitle, 0.6)
